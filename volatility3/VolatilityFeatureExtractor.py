...
def extract_pslist_features(jsondump):
    ...
    return {
        ...
        'pslist.avg_threads': sum(int(p['Threads']) for p in procs) / len(procs),
        # # of 64-bit processes
        # TODO what about Linux?
        # 'pslist.nprocs64bit': sum(p['Wow64'] for p in procs),
        'pslist.nprocs64bit': sum(1 for p in procs if p['Wow64']),
        # Avg. handler count
        # 'pslist.avg_handlers': sum(p['Hnds'] for p in procs) / len(procs),
        # 'pslist.avg_handlers': sum(int(p['Handles']) for p in procs) / len(procs),
        'pslist.avg_handlers': sum(int(p['Handles']) if p['Handles'] != '-' else 0 for p in procs) / len(procs),
...
def extract_handles_features(jsondump):
    ...
    # ver3 :PID     Process Offset  HandleValue     Type    GrantedAccess   Name
    ...
    return {
        ...
        #★SEOTO ここで下記11個以外のTypeもあるがそれらは調べなくていいのか？
...
def extract_malfind_features(jsondump):
    ...
    # print('malfind.protection', sum(m['Protection'].count(permission) for m in malfind for permission in ['EXECUTE', 'READ', 'WRITE']))
    return {
        ...
        'malfind.commitCharge': sum(int(m['CommitCharge']) for m in malfind),
        #★SEOTO　Protectionは　RWX(PAGE_EXECUTE_READWRITE)場合は３カウントか？
        'malfind.protection':  sum(m['Protection'].count(permission) for m in malfind for permission in ['EXECUTE', 'READ', 'WRITE']),
...
def extract_svcscan_features(jsondump):
    #ver3: Offset	Order	PID	Start	State	Type	Name	Display	Binary
...
VOL_MODULES = {
    ...
    # 14件
    # 'psxview': extract_psxview_features,  #ver3でなくなっていた。ー＞　とりあえず保留
    # 0件
    # 'connections': extract_connections_features, #ver3でなくなった
    # 0件
    # 'sockets': extract_sockets_features, #ver3でなくなった
    'modules': extract_modules_features,
    'svcscan': extract_svcscan_features,
    'callbacks': extract_callbacks_features,
    # 0件
    # 'apihooks': extract_apihooks_features, #ver3でなくなった
}
...
def parse_args():
    ...
    p.add_argument('-V', '--volatility-exe', default='volatility3', help='Name of volatility executable.')
...
def normalize_data(header, data):
    normalized_data = []
    ...
    for row in data:
        ...
        for i, value in enumerate(row):
            # ここで各列に対する正規化の処理を実装します
            # 例えば、数値列の場合は float() で変換したり、文字列列の場合は小文字に変換したりします
            normalized_row[header[i]] = value
...
def invoke_volatility(volatility_exe, memdump_path, module, output_to):
    ...
    # print(volatility_exe + ' -f ' + memdump_path + ' --output=json' + ' --profile=Win10x64' + ' --output-file ' + output_to + ' ' + module)

    command = ['python3',volatility_exe, '-f', memdump_path, '-j', output_to, 'windows.'+ module]
    print(command)
    subprocess.run(command)
   
    
# Pls find your OS profile and replace in this tag '--profile=Win7SP1x86' and be sure that Volatility will support the listed features in your particular profile
...
def extract_all_features_from_memdump(memdump_path, output_to, volatility_exe):
    ...
    with tempfile.TemporaryDirectory() as workdir:
        ...
        print("VOL_MODULES.items()", VOL_MODULES.items())
